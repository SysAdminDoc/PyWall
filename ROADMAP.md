...
Windows Firewall GUI + network monitor (~6.9k lines PyQt6, single file). Roadmap targets background-service mode, per-process accounting, deeper threat intel, and multi-host management.

## Planned Features

### Background Service
- Windows Service mode — monitor and auto-block without the GUI process
- IPC between service and tray GUI (named pipe or localhost TLS socket)
- Headless config reload on file change
- Resume from last state on crash / reboot

### Accounting
- Per-process byte counters (psutil `io_counters` + ETW fallback)
